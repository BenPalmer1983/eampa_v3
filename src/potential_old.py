...
class potential:
  ...
  def load():
    ...
    # For Fitting
    potential.embedding_range_factor = 1.0
    if('embedding_range_factor' in g.inp['potential'].keys()):
      try:
        potential.embedding_range_factor = float(g.inp['potential']['embedding_range_factor'])
      except:
        potential.embedding_range_factor = 1.0
    ...
    #  potential.rescale_density_on = True
    # potential.rescale_embedding_on = False
    ...
    potential.pf_output_file(g.dirs['wd'] + '/data/start.pot')
    #main.end()   
    ...
    for fn in range(len(g.pot_functions['functions'])): 
      if(g.pot_functions['functions'][fn]['fit_file'] != None):
        ...
        if(os.path.isfile(fit_file)):
          ...
          if(params_start != None and vr != None and params_lower == None):
            params_lower = []
            for i in range(len(params_start)):
              params_lower.append(str(float(params_start[i]) - vr * float(params_start[i])))
          if(params_start != None and vr != None and params_upper == None):
            params_upper = []
            for i in range(len(params_start)):
              params_upper.append(str(float(params_start[i]) + vr * float(params_start[i])))
  ...
  def pf_output(): 
    if(g.outputs):     
      potential.pf_output_file(g.dirs['wd'] + '/data/potential.dat')

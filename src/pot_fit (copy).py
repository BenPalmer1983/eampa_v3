...
class pf:
  ...
  def cycle(c, c_id, ct_id):
    ...
    for gen in range(gens):
      ...
      """
      m = 0.5
      m_inc = (10.0-0.5) / (pf.fresh_size - 1)
      for p_fresh in range(pf.pop_size_d, pf.pop_size_d + pf.fresh_size):          
        pf.ps[p_fresh,:] = pf.random_p(pf.ps[-5,:], m)
        m = m + m_inc
      for p_fresh in range(pf.pop_size_d + pf.fresh_size, pf.pop_size_d + pf.fresh_size + pf.fresh_size): 
        rn = numpy.random.randint(pf.pop_size_d)
        pf.ps[p_fresh,:] = pf.random_p(pf.ps[rn,:], m)
        m = m + m_inc
      """
  ...
  def check_improvement(params, rss): 
    ...
    if(pf.rss[-5] < 0 or rss < pf.rss[-5]):
      ...
      #pf.bulk_properties[:,31:40] = pf.bulk_properties[:,21:30]

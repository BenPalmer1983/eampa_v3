SUBROUTINE sort_1d_dp()
!#############################################################
!#############################################################
CALL CPU_TIME(start_time)
IF(make_key_table)THEN
  IF(sort_ascending)THEN
    CALL msort_1d_dp_asc_kt(arr_dp_1d, key_table)
  ELSE
    CALL msort_1d_dp_desc_kt(arr_dp_1d, key_table)
  END IF
ELSE
  IF(sort_ascending)THEN
    CALL msort_1d_dp_asc(arr_dp_1d)
  ELSE
    CALL msort_1d_dp_desc(arr_dp_1d)
  END IF
END IF
...
RECURSIVE SUBROUTINE msort_1d_dp_asc(arr)
!#############################################################
...
IF(SIZE(arr,1) .GT. 1)THEN
  ...
  CALL msort_1d_dp_asc(arr(1:lhs_len))
  CALL msort_1d_dp_asc(arr(lhs_len+1:SIZE(arr,1)))
...
RECURSIVE SUBROUTINE msort_1d_dp_asc_kt(arr, kt)
!#############################################################
REAL(kind=DoubleReal), INTENT(INOUT) :: arr(:)
...
IF(SIZE(arr,1) .GT. 1)THEN
  ...
  CALL msort_1d_dp_asc_kt(arr(1:lhs_len), kt(1:lhs_len))
  CALL msort_1d_dp_asc_kt(arr(lhs_len+1:SIZE(arr,1)), kt(lhs_len+1:SIZE(arr,1)))
  ...
  END IF  
END IF
END SUBROUTINE msort_1d_dp_asc_kt




RECURSIVE SUBROUTINE msort_1d_dp_desc_kt(arr, kt)
!#############################################################
REAL(kind=DoubleReal), INTENT(INOUT) :: arr(:)
INTEGER(KIND=StandardInteger), INTENT(INOUT) :: kt(:)
!#############################################################
...
IF(SIZE(arr,1) .GT. 1)THEN
  ...
  CALL msort_1d_dp_desc_kt(arr(1:lhs_len), kt(1:lhs_len))
  CALL msort_1d_dp_desc_kt(arr(lhs_len+1:SIZE(arr,1)), kt(lhs_len+1:SIZE(arr,1)))
  ...
  END IF  
END IF
END SUBROUTINE msort_1d_dp_desc_kt

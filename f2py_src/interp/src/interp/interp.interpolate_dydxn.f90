SUBROUTINE interpolate_dydxn(xi, x, y, dn, n_interp_in, y_out)
!############################################################
IMPLICIT NONE
!############################################################
! IN/OUT
...
INTEGER(kind=StandardInteger), INTENT(IN) :: dn
...
CALL interpndydxn(xi, x(n:n+n_interp-1), y(n:n+n_interp-1), dn, y_out)

END SUBROUTINE interpolate_dydxn 
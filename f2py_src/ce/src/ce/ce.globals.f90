...
INTEGER(kind=StandardInteger) ::                        v_ghost_size = 4000000
INTEGER(kind=StandardInteger) ::                        v_nl_size = 5000000

! Log the sizes
INTEGER(kind=StandardInteger) ::                        l_config_size = 0
INTEGER(kind=StandardInteger) ::                        l_ghost_size = 0
...
INTEGER(kind=StandardInteger) ::                        calc_count = 0
INTEGER(kind=StandardInteger) ::                        key(1:max_configs, 1:32)
INTEGER(kind=StandardInteger) ::                        cc_log(1:max_configs, 1:32)   ! 1 eos start,  2 eos end, 3 ec11 start, 4ec11 end ...
...
INTEGER(kind=StandardInteger) ::                        bp_keys_i(1:100, 1:10) = -1
...
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_b0p(:)
...
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_b0_r_gpa(:)  
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_b0_v_gpa(:)  
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_b0_avg_gpa(:)  
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_g_r_gpa(:)  
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_g_v_gpa(:)  
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_g_avg_gpa(:) 
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_g_vec_gpa(:,:)  
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_e_gpa(:)  
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_e_vec_gpa(:,:)  


! Named Values
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_cubic_cauchy_pressure(:)
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_cubic_tetragonal_shear(:)
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_cubic_shear_modulus(:)
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_cubic_cauchy_pressure_gpa(:)
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_cubic_tetragonal_shear_gpa(:)
REAL(kind=DoubleReal), ALLOCATABLE ::                   calc_cubic_shear_modulus_gpa(:)

! Stability
INTEGER(kind=StandardInteger), ALLOCATABLE ::           calc_cubic_stability(:)
INTEGER(kind=StandardInteger), ALLOCATABLE ::           calc_stability(:)




REAL(kind=DoubleReal) ::                                rss_weights(1:10) = 1.0D0  
REAL(kind=DoubleReal) ::                                rss_weights_neg_ec = 1.0D0

REAL(kind=DoubleReal), ALLOCATABLE ::                   rss(:, :)
REAL(kind=DoubleReal), ALLOCATABLE ::                   rss_total(:)
REAL(kind=DoubleReal) ::                                rss_by_type(1:10) = 0.0D0  
...
REAL(kind=DoubleReal) ::                                rss_by_type_w(1:10) = 0.0D0 
REAL(kind=DoubleReal) ::                                rss_total_rss_w = 0.0D0       ! <<< The total of all configs (weighted)

INTEGER(kind=StandardInteger) ::                        residuals_n = 0
...
!INTEGER(kind=StandardInteger) ::                        rss_array_len = 0
!REAL(kind=DoubleReal) ::                                rss_total_w_array(1:10000) = 0.0D0   ! <<< sum should match rss_total_rss_w

...
IF(crystal_type_in .EQ. 1)THEN
  ...
  CALL add_inner(rcut_in, alat_in, uv_in, label_in, crystal_type_in, expansion_in, atoms_per_crystal, &
                 primitive_labels(1:1), primitive_coords(1:1,1:3), bp_id)
! BCC
ELSE IF(crystal_type_in .EQ. 2)THEN
  ...
  CALL add_inner(rcut_in, alat_in, uv_in, label_in, crystal_type_in, expansion_in, atoms_per_crystal, &
                 primitive_labels(1:2), primitive_coords(1:2,1:3), bp_id)
...
ELSE IF(crystal_type_in .EQ. 3)THEN
  ...
  CALL add_inner(rcut_in, alat_in, uv_in, label_in, crystal_type_in, expansion_in, atoms_per_crystal, &
                 primitive_labels(1:4), primitive_coords(1:4,1:3), bp_id)
...
ELSE IF(crystal_type_in .EQ. 4)THEN
  ...
  CALL add_inner(rcut_in, alat_in, uv_in, label_in, crystal_type_in, expansion_in, atoms_per_crystal, &
                 primitive_labels(1:8), primitive_coords(1:8,1:3), bp_id)
...
REAL(kind=DoubleReal) :: alat_temp
REAL(kind=DoubleReal) :: uv_temp(1:3,1:3)
...
DO WHILE(bp_keys_i(n, 1) .NE. -1)
  IF(bp_keys_i(n, 1) .EQ. label .AND. &
     ...
     bp_keys_i(n, 3) .EQ. expansion .AND. &
...
IF(bp_id .EQ. 0)THEN
  ...
  alat_temp = alat_in * maxval(uv_in)
  uv_temp(1:3,1:3) = uv_in / maxval(uv_in)
  ...
  !# STORE UNIQUE DETAILS - LABEL, TYPE, EXPANSION, RCUT, ALAT
  bp_keys_i(bp_id, 1) = label
  bp_keys_i(bp_id, 2) = crystal_type
  bp_keys_i(bp_id, 3) = expansion
  bp_keys_r(bp_id,1) = rcut_in
  bp_keys_r(bp_id,2) = alat_temp
  ...
  cc_log(bp_id, 1) = cc_start
  cc_log(bp_id, 2) = cc_end
  ...
  DO n=1,eos_points
    ...
    ! Calc volume
    vol_cell(:,:) = alat_temp * uv_config(:,:)
    vol = TripleProduct(vol_cell(1,:), vol_cell(2,:), vol_cell(3,:))

    CALL add_config(rcut_in, alat_temp, uv_config, expanded_labels(1:m), expanded_coords(1:m,1:3))  ! Increments cc by 1
    cn_counter = cn_counter + 1
    bp_configs(bp_id, cn_counter) = cc 
    CALL make_nl(cc)

    !print *, bp_id, n, cc, rcut_in, alat_temp, m
  ...
  DO ec_n = 1, 9
    cc_start = cc + 1
    cc_end = cc + ec_points  
    cc_log(bp_id, 2*ec_n+1) = cc_start
    cc_log(bp_id, 2*ec_n+2) = cc_end

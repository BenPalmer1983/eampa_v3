...
IF(ftype .EQ. 1)THEN
  IF(option_a .GT. option_b)THEN
    pkey_temp(pc, 4) = option_b     ! Type a 
    pkey_temp(pc, 5) = option_a     ! Type a 
  ELSE
    pkey_temp(pc, 4) = option_a     ! Type a 
    pkey_temp(pc, 5) = option_b     ! Type b
  END IF
ELSE
  pkey_temp(pc, 4) = option_a     ! Type a 
  pkey_temp(pc, 5) = option_b     ! f group
END IF
...
potmap = 0
DO pn =1,pc
  !print *, pn, pkey_temp(pn, 3), pkey_temp(pn, 4), pkey_temp(pn, 5)

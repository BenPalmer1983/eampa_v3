...
SUBROUTINE cubic_knot_spline_fixed_end(r, p, p_fixed, y)
!############################################################
! p coefficients
! pf r cutoffs
...
ELSE
  ...
  IF(complete .EQV. .FALSE.)THEN
    ...
    ! INTERPOLATE DERIVATIVE INPUT NODES (EXCEPT FIXED NODE AT END)
    DO n = 1, SIZE(nodes_in, 1) - 1
      j = MAX(na,MIN(n,SIZE(nodes_in, 1)-(interp_size-1)))  
      CALL cubic_knot_spline_fixed_end_interpndydxn(nodes_in(n, 1), nodes_in(j:j+(interp_size-1), 1), &
      ...
      DO i = 1, SIZE(nodes_in, 1) - 1
        DO k = 1, nsize
          ...
          ELSE          
            ...
            CALL cubic_knot_spline_fixed_end_interpn(nodes(n, 1), nodes_in(j:j+(interp_size-1), 1), &
...
END SUBROUTINE cubic_knot_spline_fixed_end



! VECTOR SUBROUTINE
SUBROUTINE cubic_knot_spline_fixed_end_v(r, p, p_fixed, y)
!############################################################
! PAIR SPLINE
IMPLICIT NONE
!############################################################
...
DO n = 1, SIZE(r,1)
  CALL cubic_knot_spline_fixed_end(r(n), p, p_fixed, y(n))
END DO
END SUBROUTINE cubic_knot_spline_fixed_end_v




SUBROUTINE cubic_knot_spline_fixed_end_interpn(xi, x, y, yi)
...
END IF
!############################################################
END SUBROUTINE cubic_knot_spline_fixed_end_interpn


SUBROUTINE cubic_knot_spline_fixed_end_interpndydxn(xi, x, y, interp_n, y_out)
...
END IF
!############################################################
END SUBROUTINE cubic_knot_spline_fixed_end_interpndydxn

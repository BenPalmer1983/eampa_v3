  ...
  def run():
    for fn in range(len(g.pot_functions['functions'])): 
      if(g.pot_functions['functions'][fn]['f_type_id'] == 2):
        ...
        g.pot_functions['functions'][fn]['points'][:,1:3] = (0.9 / rho) * g.pot_functions['functions'][fn]['points'][:,1:3]
        rho = rescale_density.estimate_density(fn)
...
class pf_data:
  ...
  def make():
    ...
    d = {
        'stage': 'Not Set', 
        'sane': {'seeds': numpy.zeros((sane_density_seeds, width,),),
        },
        'params': {'count': width, 
...
class pf_densities:
  ...
  def run():
    ...
    pn = 0
    while(pn < len(g.pfdata['sane']['seeds'][:,0])):
      params = pf_cycle.random_p()
      pf_potential.update(params)
      for fn in range(len(g.pot_functions['functions'])): 
        if(g.pot_functions['functions'][fn]['f_type_id'] == 2):
          rho = pf_densities.estimate_density(fn)
          if( rho >= -1.0 and rho <=2.0):
            g.pfdata['sane']['seeds'][pn,:] = params
            pn = pn + 1

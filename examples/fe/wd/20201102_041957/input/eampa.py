  ...
  def make_spline_points():  
    for i in range(len(g.pot_functions['functions'])):    
      if(g.pot_functions['functions'][i]['function_type'] == 3):
        st = 1   # poly3
        if(g.pot_functions['functions'][i]['s_type'] == 'poly5'):
          st = 2
      
        splined = spline.spline_nodes(st, g.pot_functions['functions'][i]['s_nodes'][:,:], 100)
        g.pot_functions['functions'][i]['points'] = interp.fill(splined[:,0], splined[:,1], g.tab_size, g.tab_width)
...
# Print start screen
    ...
    potential.plot_python_potentials(g.dirs['wd'] + '/plots/pot/start_potential')
    exit()
# Make Pool
...
# PRINT PARAMETERS
    ...
    for fn in range(len(g.pot_functions['functions'])):          
      if(g.pot_functions['functions'][fn]['fit_type'] == 1):     # SPLINE
        ...
        for i in range(g.pot_functions['functions'][fn]['fit_size']):
          print("  [" + str(i) + "]", display.pad_r(g.pot_functions['functions'][fn]['s_params'][i],8), end='')
        print()
      elif(g.pot_functions['functions'][fn]['fit_type'] == 2):   # ANALYTIC
